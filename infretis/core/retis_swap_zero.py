def retis_swap_zero(ensembles, settings, cycle):
    ...
    # 1. Check Metropolis acceptance rule based on energies 
    # We don't perform any integrations yet, but we need to 
    # make the phase_point x_lo from engine0 compatible with engine1,
    # and the phase_point x_hi from engine1 compatible with engine0 
    logger.debug('Swapping [0^-] <-> [0^+]')
    logger.debug('Converting phasepoint x_lo for engine1')
