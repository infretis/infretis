...
def wirefence_weight_and_pick(path, intf_l, intf_r, return_seg=False, ensemble=None):
    ...
    if return_seg and n_frames:
        sum_frames = 0
        subpath_select = ensemble.rgen.rand()
        for i in path_arr:
            sum_frames += i[2]
...
def shoot(ens_set, path, engine, shooting_point=None, start_cond=('L',)):
    ...
    if ('L' not in set(ens_set['start_cond']) and
    ...
    if set(('R', 'L')) == set(ens_set['start_cond']):
        pass
...
def wire_fencing(pens, shooting_point=None, start_cond=('L',)):
    ...
    n_frames, new_segment = wirefence_weight_and_pick(trial_path, wf_int[0],
                                                      wf_int[2],
                                                      return_seg=True, ensemble=ensemble)
    ...
    succ_seg = 0
    for i in range(ensemble.mc_move.get('n_jumps', None)):
...
def retis_swap_zero(picked):
    ...
    if path0.length == maxlen0:
        ...
        print('tiger', len(path_tmp), [i.order[0] for i in path_tmp.phasepoints])
        path0.status = 'BTS'
