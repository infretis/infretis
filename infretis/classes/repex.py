...
class REPEX_state:
    ...
    def initiate_ensembles(self):
        ...
        # Per-ensemble adaptive parameter overrides (for restart fidelity).
        # Written by treat_output() after each move; applied here on init/restart.
        ensemble_nsubpath = self.config["simulation"].get(
            "ensemble_nsubpath", []
        )
        ensemble_mwf_nsubpath = self.config["simulation"].get(
            "ensemble_mwf_nsubpath", []
        )

        # create all path ensembles
        pensembles = {}
        for i, ens_intf in enumerate(ens_intfs):
            tis_copy = copy.deepcopy(self.config["simulation"]["tis_set"])
            if i < len(ensemble_nsubpath):
                tis_copy["n_jumps"] = ensemble_nsubpath[i]
            if i < len(ensemble_mwf_nsubpath):
                tis_copy["mwf_nsubpath"] = ensemble_mwf_nsubpath[i]
            pensembles[i] = {
