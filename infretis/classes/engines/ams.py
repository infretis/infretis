...
class AMSEngine(EngineBase, metaclass=Singleton):
    ...
    def step(self, system, name, set_trajfile=True, set_step_to_zero=False): 
        ...
        """
        state, idx = system.particles.get_pos()
        ...
        # Update system
        system.particles.set_pos((new_state, idx))
        ...
        system.particles.vpot = states[-1].get_potentialenergy(unit=self.ene_unit)
        system.particles.ekin = states[-1].get_kineticenergy(unit=self.ene_unit)
    ...
    def _read_configuration(self, filename, idx=None):
        ...
        if len(box) == 0:
            ...
            out.append(box)
        out.append(state.get_coords(unit=self.dist_unit))
        out.append(state.get_velocities(dist_unit=self.dist_unit, time_unit=self.time_unit))
        return out

    def set_mdrun(self, config, md_items):
        print('AMSEnginge set_mdrun')
        """Remove or rename?"""
        self.exe_dir = md_items["w_folder"]

    def _reverse_velocities(self, filename, outfile):
        print('AMSEnginge _reverse_velocities')
    ...
    def _extract_frame(self, traj_file, idx, out_file):
        ...
        if traj_file in self.states:
            ...
            vel = np.reshape(vel, (-1, 3)) # Might be removed later if plams gets updated to match AMSWorker
    ...
    def _propagate_from(self, name, path, ensemble, msg_file, reverse=False):
        print('AMSEnginge _propagate_from')
        """
        ...
        initial = ensemble['system'].particles.get_pos()[0]
        ...
        for i in range(path.maxlen):
            ...
            ensemble['system'].particles.set_pos((traj_file, i))
            out = self._read_configuration(traj_file, idx=i)
            order = self.calculate_order(ensemble, xyz=out[1], vel=out[2], box=out[0])
            ...
            phase_point = self.snapshot_to_system(ensemble['system'], snapshot)
    ...
    def modify_velocities(self, ensemble, vel_settings):
        print('AMSEnginge modify_velocities')
        ...
        if vel_settings.get('aimless', False):
            state_name, idx = system.particles.get_pos()
            logger.debug('Generating velocities for %s, idx=%s', state_name, idx)

            genvel = os.path.join(self.exe_dir, "gen_vel." + self.ext)
            ...
            system.particles.set_pos((genvel, None))
            system.particles.set_vel(False)
            system.particles.ekin = kin_new
            system.particles.vpot = state.get_potentialenergy(unit=self.ene_unit)
