...
def calculate_minutes_since_start(datetime_array):
    ...
    datetimes = np.array([datetime.strptime(dt, '%Y.%m.%d %H:%M:%S') for dt in datetime_strings])

    # Calculate the time difference in minutes
    minutes_since_start = (datetimes - datetimes[0]).astype('timedelta64[m]') / np.timedelta64(1, 'm')
