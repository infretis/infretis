    ...
    with open(inp, 'r') as read:
        for idx, line in enumerate(read):
            ...
            if key not in w_data:
                w_data[key] = {'md_start': [], 'wmd_start': [],
                               'wmd_end': [], 'md_end': [],
                               'dask_end': [], 'enss': [], 'connect': []}
    ...
    for worker in w_data.keys():
        ...
        for idx, wstart in enumerate(w_data[worker]['md_start']):
            ...
            for ens in w_data[worker]['enss'][idx]:
                plt.plot([wstart, wend], [ens]*2, color=c_dic[worker],
                         linewidth='2.')
                if subtime:
                    plt.scatter([wmds, wmde, mde], [ens]*3, color='k', marker='x', linewidths=1., alpha=1.0)
                plt.scatter([wstart, wend], [ens]*2, color=c_dic[worker], marker='|', linewidth=2.)

            # vertical lines
            if w_data[worker]['connect'][idx] != '-':
                enss = w_data[worker]['connect'][idx].split('-')
                plt.plot([wstart]*2, [int(i) for i in enss], '--',
                         color=c_dic[worker], linewidth='2.', alpha=0.4)
        ...
        if scatter:
            ...
            for idx, d_end in enumerate(w_data[worker]['dask_end']):
                ...
                for ow in other_w:
                    for start, end, ens in zip(w_data[ow]['md_start'],
                                               w_data[ow]['dask_end'],
                                               w_data[ow]['enss']):
                        if start < d_end < end:
                            for ens0 in ens:
                ...
                plt.scatter([d_end]*len(free_ens), free_ens,
                             color=c_dic[worker], alpha=0.4,
                             marker=r's')
