...
from infretis.classes.engines.lammps import read_lammpstrj, write_lammpstrj
...
for ens_name in state.ensembles.keys():
    ...
    ensemble["allowmaxlength"] = True  # should be in simulation not tis_set
    ...
    while status != "ACC":
        if ens_name == 0:
            success, new_path, status = shoot(
                ensemble, path, engine, shooting_point=system, start_cond="R"
            )
        else:
            success, new_path, status = shoot(
    ...
    io = PathStorage()
    io.output_path_files(str(ens_name), [new_path, status], path_dir)
